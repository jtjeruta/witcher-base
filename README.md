# Witcher Base

Turn your colonists into witchers. This is the foundation mod of a modular Witcher content suite for **RimWorld**, letting you put pawns through the legendary trials that grant inhuman abilities — if they survive.

> **Requires the Biotech DLC.**

## What it does

Research the witcher trials, then perform them on colonists as operations from the **Health** tab. Each trial costs herbal ingredients and throws the pawn into a brutal fever that lasts a day or two. Survivors come out changed — their xenotype is replaced with the next stage of witcher training. Those who don't survive... don't.

You can also pick any witcher xenotype when creating starting colonists. Witchers **never spawn randomly** in the world.

## Witcher xenotypes

Witchers progress through four xenotypes in order:

| Xenotype | How to obtain |
|----------|---------------|
| **Witcher initiate** | Survive the Trial of Grasses |
| **Witcher** | Survive the Trial of Dreams |
| **Master witcher** | Complete a witcher contract (Trial of Mountains) |
| **Mutated witcher** | Survive Additional Mutagens |

Each stage replaces the pawn's full gene set with the next tier — genes are overridden, not stacked.

## The trials

### Trial of Grasses
The first and most infamous initiation — a brutal ordeal that most who attempt it do not survive. Those who endure become **witcher initiates**.

**Survival rate:** about 3 in 10.

### Trial of Dreams
A second trial, open only to witcher initiates. Far more forgiving than the first, it rewires the mind.

**Survival rate:** about 7 in 10. Becomes a **witcher**.

### Trial of Mountains
The final trial. A witcher can take **witcher contracts** — beasts sighted on nearby tiles that must be hunted down and killed.

**Survival rate:** depends on the hunt. Becomes a **master witcher**.

### Additional Mutagens
An experimental and exceptionally deadly procedure for master witchers only. Very few survive.

**Survival rate:** about 1 in 10. Becomes a **mutated witcher**.

## Compatibility

- Built for **RimWorld 1.6**.
- Requires **Biotech** (the trials grant Biotech genes via xenotypes).
- Safe to add to an existing save. Removing it mid-save is not recommended once colonists have undergone trials.

## Credits

A fan-made tribute. The Witcher universe and its lore belong to CD Projekt Red and Andrzej Sapkowski.
